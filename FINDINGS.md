...
near-square Cityscapes sign polygons (5 cities, train split).
No detector involved; ground-truth boxes only, to isolate the
geometric estimator from detection error.

| box h (px) | n   | median Z_true | bias  | p25  | p75  | MAE  |
|------------|-----|---------------|-------|------|------|------|
...
across 20-70 m. MAE is flat (19-22%) over a 3.5x change in range,
so error is dominated by sign-size ambiguity, not pixel quantization —
consistent with the -12%/+30% floor measured independently in Stage 1.
...
being disproportionately Größe 1 (420 mm), for which 650 mm overestimates
apparent size and therefore underestimates range.
